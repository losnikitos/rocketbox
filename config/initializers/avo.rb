...
Avo.configure do |config|
  ...
  config.current_user_method do
    Session.find_by(id: cookies.signed[:session_token])&.user
  end

  config.authenticate_with do
    user = Session.find_by(id: cookies.signed[:session_token])&.user
    if user.nil?
      redirect_to main_app.sign_in_path
    elsif !user.admin?
      redirect_to main_app.root_path, alert: "You do not have access to the admin."
    end
  end

  ## == Authorization ==
  # Admin-only gate is enforced via authenticate_with (above). CRUD does not use Pundit.
