...
class AccountsController < ApplicationController
  ...
  def subscription_status
    unless Current.user.admin?
      redirect_to subscription_path, alert: "You are not allowed to update subscription status."
      return
    end

    status = params.require(:subscription_status)
    unless ADMIN_EDITABLE_SUBSCRIPTION_STATUSES.include?(status)
      redirect_to subscription_path, alert: "Unknown subscription status."
      return
    end
    ...
    redirect_to subscription_path, notice: "Subscription status updated."
  end
end
