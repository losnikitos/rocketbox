...
class AccountsController < ApplicationController
  ...
  def checkout
    ...
    if price_id.blank?
      redirect_to account_subscription_path, alert: "Subscription billing is not configured."
      return
    end

    if Stripe.api_key.blank?
      redirect_to account_subscription_path, alert: "Stripe is not configured."
      return
    end
    ...
    redirect_to account_subscription_path, alert: "Could not start checkout. Please try again."
  end

  def portal
    sub = Current.user.subscription
    if sub.stripe_customer_id.blank?
      redirect_to account_subscription_path, alert: "Subscribe first to manage billing."
      return
    end
    ...
    redirect_to account_subscription_path, alert: "Could not open billing portal. Please try again."
  end

  def subscription_status
    unless Current.user.admin?
      redirect_to account_subscription_path, alert: "You are not allowed to update subscription status."
      return
    end

    status = params.require(:subscription_status)
    unless ADMIN_EDITABLE_SUBSCRIPTION_STATUSES.include?(status)
      redirect_to account_subscription_path, alert: "Unknown subscription status."
      return
    end
    ...
    redirect_to account_subscription_path, notice: "Subscription status updated."
  end
end
